...
int RequestBuffer::_first_line_handler()
{
	...
	if (!IS_METHOD_SUPORTED(_method))	return 5010; // not implemented.
...
// #define TMP_FILE_PATH "/Users/moulmoud/goinfre/tmp"
...
std::string RequestBuffer::_generate_tmp_file_path()
{
	...
	if (!(stat( "/Users/moulmoud/goinfre/tmp", &info) == 0 && S_ISDIR(info.st_mode)))
	{
		if (mkdir( "/Users/moulmoud/goinfre/tmp", 0777) == -1)
		{
	...
	while (true)
	{
		_path = "/Users/moulmoud/goinfre/tmp/file_";
		std::stringstream ss;
		ss << i++;
		_path += ss.str();
