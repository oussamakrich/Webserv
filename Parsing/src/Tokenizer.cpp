...
void Tokenizer::validateTokens(TOKEN_STRUCTS &tokens)
{
	...
	for (unsigned int i = 0; i < size; i++)
	{
		if (i < size && tokens[i].type == SEMICOLON)
		{
			...
			{
				std::cerr << SYNTAX_ERROR << tokens[i].column << ":" << tokens[i].row << RESET" expected word or quotes after semicolon.\n";
		...
		else if (i < size && tokens[i].type == OPEN_C_BRACKET)
		{
			...
			{
				std::cerr << SYNTAX_ERROR << tokens[i].column << ":" << tokens[i].row << RESET" empty blocks are not allowed.\n";
				std::cerr << RED"\n\t\tError code: ("<< (EMPTY_BLOCK + 1)  <<  ")" << RESET <<"\n";
				exit(1);
			}
			if (i < size && tokens[i].type == SEMICOLON)
			{
				std::cerr << SYNTAX_ERROR << tokens[i].column << ":" << tokens[i].row << RESET" expected word or quotes before semicolon.\n";
				std::cerr << RED"\n\t\tError code: ("<< (MISSING_SEMICOLON + 1)  <<  ")" << RESET <<"\n";
				exit(1);
			}
		}
...
void Tokenizer::QuotesHandler(std::string &line, int lineNumber, unsigned int  &i, TOKEN_STRUCTS &tokens, char quote)
{
	...
	{
		if (quote == '\"')
			std::cerr << SYNTAX_ERROR << lineNumber << ":" << i << RESET" Empty Double Quotes. \n";
		else
			std::cerr << SYNTAX_ERROR << lineNumber << ":" << i << RESET" Empty Single Quotes. \n";
...
void Tokenizer::fatalError(Error error,TOKEN_ITERATOR &i, std::string str)
{
	...
	{
		std::cerr << SYNTAX_ERROR << 0 << ":" << 0 << str ;
		std::cerr << RED"\t\tError code: ("<< (error + 1) << ")\n" << RESET;
	}
	else
	{
		std::cerr << SYNTAX_ERROR << i->column << ":" << (i)->row << str << i->value << ".\n\n" ;
		std::cerr << RED"\t\tError code: ("<< (error + 1) << ")\n" << RESET;
	}
	exit(error + 1);
}
